...
/*
LOCKED PAGE RULE (BUILD-SAFE):
PAGE 1: Title only
PAGE 2: Scripture + Illustration together (everything before MAIN POINT)
...
If manuscript is plain text, we wrap Title + one page (best possible).
*/
...
function buildPagedHtmlFromHtml(title, scriptureRef, html) {
  // ✅ BUILD-SAFE: During Lovable build there is no DOM/window
  if (typeof window === "undefined") {
    return wrapPlainTextAsHtmlPages(title, scriptureRef, html);
  }

  // Guard DOMParser just in case
  ...
  const bodyNodes = Array.from(doc.body.childNodes).filter((n) => {
    const isText = n && n.nodeType === 3; // TEXT_NODE (no Node global)
...
<section class="pdf-page title-page">
  ...
  // If no MAIN POINT headings, put all content on Page 2
...
<section class="pdf-page scripture-illustration-page">
  ...
  const beforeWrap = doc.createElement("div");
...
<section class="pdf-page scripture-illustration-page">
  ...
  for (let i = 0; i < mainPointIndexes.length; i++) {
    ...
    const chunkWrap = doc.createElement("div");
...
<section class="pdf-page point-page">
  ...
  if (closingIndex !== -1) {
    ...
    const closingWrap = doc.createElement("div");
...
@media print {
  ...
  /* Make each wrapper act like a full A4 page */
