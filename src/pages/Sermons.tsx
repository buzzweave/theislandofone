    ...
    <div className="min-h-screen">
      <section className="py-20 bg-gradient-section">
        <div className="container mx-auto px-4 text-center">
          ...
          <h1 className="font-display text-5xl md:text-6xl font-bold mb-4">Sermon Library</h1>
            ...
            {accessFilters.map((af) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium uppercase tracking-wider transition-colors ${
                  accessFilter === af ? "bg-primary/20 text-primary border border-primary/30" : "bg-card border border-border text-muted-foreground hover:text-foreground"
                }`}
              >
                {af === "All" ? "All Access" : af}
              </button>
            ))}
          </div>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <h3 className="font-display text-xl font-semibold mb-1 group-hover:text-primary transition-colors">{sermon.title}</h3>
