...
import { useMediaFolders, useMediaImages, type MediaFolder } from "@/hooks/useMediaLibrary";
...
export default function GraphicsFolders({ orgId }: { orgId: string }) {
  const { toast } = useToast();
  const { data: folders = [], isLoading } = useMediaFolders(orgId);
...
function FolderDetail({
  ...
  const { data: images = [], isLoading } = useMediaImages(orgId, folder.id);
